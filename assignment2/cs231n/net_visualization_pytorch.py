...
def compute_saliency_maps(X, y, model):
    ...
    loss.backward()
    saliency = torch.abs(torch.max(X.grad, axis=1)[0])
...
def make_fooling_image(X, target_y, model):
    ...
    for i in range(500):
        ...
        loss = torch.nn.CrossEntropyLoss()(score, y)
        model.zero_grad()
        loss.backward()
        ...
        if target_y == torch.max(score, dim=1)[1][0].item():
            break
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ##############################################################################
    #                             END OF YOUR CODE                               #
...
def class_visualization_update_step(img, model, target_y, l2_reg, learning_rate):
    ...
    scores = model(img)
    target_score = scores[0, target_y] - l2_reg * torch.norm(img)
