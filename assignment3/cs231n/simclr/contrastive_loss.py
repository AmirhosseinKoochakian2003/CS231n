...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        total_loss += -torch.log(torch.exp(sim(z_k, z_k_N) / tau) / torch.sum(torch.tensor([torch.exp(sim(out[k], out[i]) / tau) for i in range(2*N) if i != k])))
        total_loss += -torch.log(torch.exp(sim(z_k_N, z_k) / tau) / torch.sum(torch.tensor([torch.exp(sim(out[k+N], out[i]) / tau) for i in range(2*N) if i != k+N])))
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    exp_sim_pairs = torch.exp(sim_pairs / tau)
    numerator = torch.cat([exp_sim_pairs, exp_sim_pairs], dim=0)
